...
import { MapPin, Clock, Calendar, AlertTriangle, ShieldAlert, BadgeCheck, Car, Landmark, Trash2, Power, Plus, ArrowRight, FileText, Sparkles, Church, Users, CheckCircle2 } from 'lucide-react';
...
export default function DriverDashboard() {
  ...
  const handlePostRide = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Graceful fallback: If any newly added columns do not exist in live Supabase DB, retry with base columns
      if (postError && (postError.code === '42703' || postError.message?.includes('church') || postError.message?.includes('ride_purpose') || postError.message?.includes('service_name') || postError.message?.includes('notes') || postError.message?.includes('community_name'))) {
        console.warn('New schema columns not found in database, retrying insert with standard columns:', postError.message);
        ...
        if (postCommunity && postCommunity.trim()) {
          basePayload.community_name = postCommunity.trim();
        }
