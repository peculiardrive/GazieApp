...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    // Check if we have an active auth session or hash
    supabase.auth.onAuthStateChange(async (event: any, session: any) => {
      if (event === 'PASSWORD_RECOVERY') {
        // User arrived from recovery link
        setMessage({ text: 'Please enter your new password below.', isError: false });
      }
    });
  }, []);
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        setSuccess(true);
        setMessage({ text: 'Your password has been successfully updated!', isError: false });
        setTimeout(() => {
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs text-gazie-navy/70 max-w-xs mx-auto font-medium">
              {success 
                ...
                : 'Create a new, strong password for your Gazie Commute account.'
              }
            </p>
          </div>
